...
func CreateEndorsementOnlyWorkers(ctx context.Context, crypto infra.Crypto, raw chan *peer.Proposal, signed []chan *basic.Elements, envs chan *common.Envelope, processed chan *basic.Elements, config basic.Config, num int, burst, signerNumber int, rate float64, logger *log.Logger, errorCh chan error) ([]infra.Worker, error) {
	generator_workers := make([]infra.Worker, 0)
	proposers, err := CreateProposers(ctx, signed, processed, config, logger)
	if err != nil {
		return generator_workers, err
